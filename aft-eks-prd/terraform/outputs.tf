output "eks_blueprints_admin_team_configure_kubectl" {
  ...
  value       = "aws eks --region ${data.aws_region.current.name} update-kubeconfig --name ${module.eks.cluster_name}  --role-arn ${module.eks_blueprints_admin_team.iam_role_arn}"
}
